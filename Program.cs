...
public static class GetStartupMainTableDataGroup2024
{
	...
	private static MatchDetails ParseMatchDetails(HtmlNode matchNode)
	{
	    if (matchNode == null) return new MatchDetails(new HalfTimeScore(0, 0), Array.Empty<MatchAction>());
	
	    // We are on the inner ".match" node; the ".details" sibling is inside the nearest ".matchitem"
	    var matchItem = AscendUntil(matchNode, n => n.GetAttributeValue("class", "").Split(' ').Contains("matchitem"));
	    var detailsNode = matchItem?.SelectSingleNode(".//div[contains(@class,'details')]");
	
	    var half = ParseHalfTime(detailsNode);
	    var actions = ParseActions(detailsNode);
	
	    return new MatchDetails(half, actions);
	}
	
	private static HtmlNode AscendUntil(HtmlNode start, Func<HtmlNode, bool> stop)
	{
	    var n = start;
	    while (n != null && !stop(n)) n = n.ParentNode;
	    return n;
	}
	
	private static HalfTimeScore ParseHalfTime(HtmlNode detailsNode)
	{
	    try
	    {
	        // .details > .info > .holder > .goals (host), .goals (guest)
	        var holder = detailsNode?
	            .SelectSingleNode(".//div[contains(@class,'info')]//div[contains(@class,'holder')]");
	
	        var goalNodes = holder?
	            .SelectNodes(".//div[contains(@class,'goals')]");
	
	        int host = 0, guest = 0;
	        if (goalNodes != null && goalNodes.Count >= 2)
	        {
	            int.TryParse(goalNodes[0].InnerText.Trim(), NumberStyles.Integer, CultureInfo.InvariantCulture, out host);
	            int.TryParse(goalNodes[1].InnerText.Trim(), NumberStyles.Integer, CultureInfo.InvariantCulture, out guest);
	        }
	...
	private static List<MatchAction> ParseActions(HtmlNode detailsNode)
	{
	    var result = new List<MatchAction>();
	    if (detailsNode == null) return result;
	
	    // Every action lives in ".action", with either ".hostteam" or ".guestteam"
	    var actionBlocks = detailsNode.SelectNodes(".//div[contains(@class,'action')]");
	    if (actionBlocks == null) return result;
	
	    foreach (var action in actionBlocks)
	    {
	        // determine side by which child exists
	        ...
	        // inside team node: .matchaction > (one child with class indicating the kind), and .player with "mm' Name"
	        var marker = teamNode.SelectSingleNode(".//div[contains(@class,'matchaction')]/div");
	        var kind = ClassToActionKind(marker?.GetAttributeValue("class", ""));
	
	        var playerRaw = teamNode.SelectSingleNode(".//div[contains(@class,'player')]")?.InnerText?.Trim() ?? "";
	
	        // typical format: "26' Boenisch S."
	        var (minute, player) = SplitMinuteAndPlayer(playerRaw);
	
	        result.Add(new MatchAction(side, kind, minute, player));
	    }
	...
	{
	    // markers observed in your HTML: .goal, .ycard, .dycard, .penalty (and possibly .rcard elsewhere)
	    if (string.IsNullOrEmpty(cls)) return ActionKind.Unknown;
	
	    if (cls.Contains("goal")) return ActionKind.Goal;
	    if (cls.Contains("penalty")) return ActionKind.Penalty;
	    if (cls.Contains("dycard")) return ActionKind.SecondYellow;
	    if (cls.Contains("ycard")) return ActionKind.YellowCard;
	    if (cls.Contains("rcard")) return ActionKind.RedCard;
	
	    return ActionKind.Unknown;
	}
	
	private static (int? minute, string player) SplitMinuteAndPlayer(string raw)
	{
	    if (string.IsNullOrWhiteSpace(raw)) return (null, string.Empty);
	
	    // Grab a leading integer before a single apostrophe, e.g., 90' Toprak O.
	    var m = Regex.Match(raw, @"^\s*(\d+)\s*'\s*(.+)$");
	    if (m.Success)
	    {
	        if (int.TryParse(m.Groups[1].Value, NumberStyles.Integer, CultureInfo.InvariantCulture, out var min))
	            return (min, m.Groups[2].Value.Trim());
	    }
	
	    // fallback: no minute parsed
