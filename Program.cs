...
// Reads AdminSecret from config (appsettings / env var ADMINSECRET).
// In production set it via environment variable: ADMINSECRET=<strong-random-value>
static bool IsAdminRequest(HttpContext ctx, IConfiguration cfg)
{
    var secret = cfg["AdminSecret"]
                 ?? Environment.GetEnvironmentVariable("ADMIN_SECRET");
    if (string.IsNullOrEmpty(secret)) return false; // no secret configured → deny
    return ctx.Request.Headers.TryGetValue("X-Admin-Secret", out var hdr)
...
app.Use(async (ctx, next) =>
{
    ...
    catch (Exception ex)
    {
        ...
        if (!ctx.Response.HasStarted)
        {
            ...
            // Do not expose stack traces to callers in production
            var msg = ctx.RequestServices.GetRequiredService<IWebHostEnvironment>().IsDevelopment()
                ? ex.ToString()
...
// Optional raw writer for scripts: PUT /app-version.txt
// Pass secret via X-App-Version-Code header (not query string, to avoid log leakage).
app.MapMethods("/app-version.txt", new[] { "PUT" }, async (HttpContext ctx) =>
{
    // Secret must be passed via X-App-Version-Code header (never in the URL query string).
    ctx.Request.Headers.TryGetValue("X-App-Version-Code", out var hv);
    var code = hv.ToString();
    var expected = Environment.GetEnvironmentVariable("APP_VERSION_CODE");
...
// POST /data/likes/recompute?hour=13 — admin only via ADMIN_SECRET header
...
{
    if (!IsAdminRequest(httpCtx, cfg)) return Results.StatusCode(403);
...
// Quick inspector to see the last N webhook posts (default 50, max 200) — admin only.
app.MapGet("/webhooks/trade/recent",
    (HttpContext ctx, [FromServices] IConfiguration cfg, [FromServices] TradeSignalStore store, [FromQuery] int take = 50) =>
{
    if (!IsAdminRequest(ctx, cfg)) return Results.StatusCode(403);
...
// Manual refresh (main page) — admin only via X-Admin-Secret header
app.MapPost("/data/refresh", async (HttpContext ctx, [FromServices] IConfiguration cfg, [FromServices] ScraperService svc) =>
{
    if (!IsAdminRequest(ctx, cfg)) return Results.StatusCode(403);
