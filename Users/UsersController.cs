...
public sealed class UsersController : ControllerBase
{
    ...
    public sealed class PublicUserProfileDto
    {
        ...
        public string? Username          { get; set; } // user_profile.display_name
        public DateTime JoinedAtUtc      { get; set; } // users.created_at
        ...
        // Activity summary
        public int CommentsCount         { get; set; }         // non-deleted comments
        public int CommentLikesReceived  { get; set; }         // likes on this user's comments
        public int LikesCastUp           { get; set; }         // user_match_votes.vote = +1
        public int LikesCastDown         { get; set; }         // user_match_votes.vote = -1
    }
    ...
    // -------------------------------
    // GET /v1/users/{idOrName}
    // Public profile (anonymous OK)
    // -------------------------------
    [HttpGet("{idOrName}")]
    [AllowAnonymous]
    public async Task<IActionResult> GetPublicProfile([FromRoute] string idOrName, CancellationToken ct)
    {
        ...
        // Resolve: numeric user_id OR display_name_norm (you already store it). :contentReference[oaicite:10]{index=10}
        ulong? userId = null;
        string? norm  = null;
...
WHERE (@uid IS NOT NULL AND u.user_id = @uid)
   OR (@norm IS NOT NULL AND p.display_name_norm = @norm)
LIMIT 1;";
        ...
        var roles = (await conn.QueryAsync<string>(@"
            SELECT r.name
             ...
             WHERE ur.user_id = @uid;", new { uid = row.UserId })).ToArray();
        ...
        // Activity counts
        // comments: exclude deleted  (comments.is_deleted)  :contentReference[oaicite:11]{index=11}
        // likes cast: user_match_votes.vote in {-1,0,1}     :contentReference[oaicite:12]{index=12}
        // likes received: count comment_likes on my comments :contentReference[oaicite:13]{index=13}
...
FROM user_follows f
JOIN user_profile p ON p.user_id = f.follower_id
...
SELECT f.followee_id AS UserId,
       ...
       f.created_at   AS FollowedAtUtc
FROM user_follows f
JOIN user_profile p ON p.user_id = f.followee_id
...
LIMIT @lim;";
    ...
    public async Task<IActionResult> Follow([FromRoute] ulong id, CancellationToken ct)
    {
        ...
        const string sql = @"
INSERT IGNORE INTO user_follows (follower_id, followee_id)
VALUES (@me, @them);";
        await using var conn = Open();
        var n = await conn.ExecuteAsync(new CommandDefinition(sql, new { me = me.Value, them = id }, cancellationToken: ct));
        return n > 0 ? Ok() : Ok(); // idempotent
    }
    ...
    // Optional: recent comments & votes (anonymous OK)
    // ------------------------------------
    public sealed class ActivityDto
    {
        public IEnumerable<object> RecentComments { get; set; } = Array.Empty<object>();
        public IEnumerable<object> RecentLikes    { get; set; } = Array.Empty<object>(); // by href (matches)
    }
    ...
    public async Task<IActionResult> GetActivity([FromRoute] ulong id, [FromQuery] int limit = 20, CancellationToken ct = default)
    {
        ...
        // recent non-deleted comments with basic context  :contentReference[oaicite:14]{index=14} :contentReference[oaicite:15]{index=15}
        var comments = await conn.QueryAsync(new CommandDefinition(@"
            ...
            LIMIT @lim;", new { uid = id, lim = limit }, cancellationToken: ct));

        // recent likes cast (by user) from user_match_votes + matches  :contentReference[oaicite:16]{index=16}
